    ...
    def __setattr__(self, name, value):
        ...
        if name in 'duration max_to_pad segment_size'.split():
            if value is not None and value <= 30:
                warnings.warn(
                    f"{name} should be in milliseconds, it looks like you might be trying to use seconds")
...
def segment_items(item, config, path):
    ...
    if not files:
        ...
        for i in range((siglen//segsize) + 1):
            ...
            if((i+1)*segsize <= siglen):
                sigs.append(sig[:, i*segsize:(i+1)*segsize])
            else:
                sigs.append(torch.cat(
                    [sig[:, i*segsize:], torch.zeros(sig.shape[0], segsize-sig[:, i*segsize:].shape[-1])], dim=1))
        files = make_cache(sigs, sr, config, "s",
                           item_path, [config.segment_size])
        _record_cache_contents(config, files)
    return list(zip(files, [label]*len(files)))
...
def _record_cache_contents(cfg, files):
    ...
    try:
        ...
        print(
            f"Unable to save files to cache log, cache at {cfg.cache_dir} may need to be cleared manually")
...
def get_outliers(len_dict, devs):
    ...
    outliers = [(k, v) for k, v in len_dict.items()
                if not (lower_thresh < v < upper_thresh)]
        ...
        if len(x.items) > 0:
            ...
            if cfg.downmix or cfg.remove_silence or cfg.segment_size or cfg.resample_to:
                ...
                if x.config.downmix:
                    ...
                    cfg._nchannels = 1
                    items = [downmix_item(i, x.config, x.path)
...
class AudioList(ItemList):
    ...
    def open(self, fn):  # file name, it seems
        fn = Path(fn)
        if self.path is not None and not fn.exists() and str(self.path) not in str(fn):
            fn = self.path/item
        if self.config.use_spectro:
            item = self.add_spectro(fn)
        else:
            func_to_add = self._get_pad_func(
            ) if self.config.max_to_pad or self.config.segment_size else None
    ...
    def add_spectro(self, fn: PathOrStr, from_item_lists=True):
        ...
        if self.config.duration and self.config._processed:
            spectro, start, end = tfm_crop_time(
                spectro, self.config._sr, self.config.duration, self.config.sg_cfg.hop_length, self.config.pad_mode)
    ...
    def create_spectro(self, item: AudioItem):
        if self.config.mfcc:
            mel = MFCC(sample_rate=item.sr, n_mfcc=self.config.sg_cfg.n_mfcc,
        ...
        if self.config.delta:
            mel = torch.cat(
                [torch.stack([m, torchdelta(m), torchdelta(m, order=2)]) for m in mel])
        return mel
    ...
    def stats(self, prec=0, devs=3, figsize=(15, 5)):
        ...
        print("Sample Rates: ")
        for sr, count in Counter(rates).items():
            print(f"{int(sr)}: {count} files")
    ...
    def from_df(cls, df: DataFrame, path: PathOrStr, cols: IntsOrStrs = 0, folder: PathOrStr = None, suffix: str = '', **kwargs) -> ItemList:
        ...
        res.items = np.char.add(np.char.add(
            pref, res.items.astype(str)), suffix)
        return res
